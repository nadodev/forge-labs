...
@section('title', 'Editar Item da Timeline')
...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-white">Editar Item da Timeline</h1>
            <p class="text-gray-400 mt-1">Atualize as informações do item</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.timeline.update', $timeline) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="subtitle" name="subtitle" value="{{ old('subtitle', $timeline->subtitle) }}"
                           class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <div>
                    ...
                    <x-icon-selector name="icon" value="{{ old('icon', $timeline->icon) }}" />
                    @error('icon')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="date" id="date" name="date" value="{{ old('date', $timeline->date?->format('Y-m-d')) }}"
                           class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('date')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $timeline->sort_order) }}"
                           class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('sort_order')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="6" 
                      class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="Descreva o que aconteceu neste marco da sua carreira...">{{ old('description', $timeline->description) }}</textarea>
            @error('description')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>

...
@section('title', 'Admin - Editar Certificado')
...
<div class="flex items-center gap-4 mb-6">
    ...
    <h1 class="text-2xl font-bold">Editar Certificado</h1>
</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-4">
            ...
            <div>
                ...
                <x-icon-selector name="icon" value="{{ old('icon', $certificate->icon) }}" />
                @error('icon')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="url" id="url" name="url" value="{{ old('url', $certificate->url) }}"
                       class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('url')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" id="issued_at" name="issued_at" value="{{ old('issued_at', $certificate->issued_at?->format('Y-m-d')) }}"
                       class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('issued_at')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $certificate->sort_order) }}"
                       class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('sort_order')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror

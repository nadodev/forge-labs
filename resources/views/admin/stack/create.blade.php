...
@section('title', 'Adicionar Tecnologia')
...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-white">Adicionar Tecnologia</h1>
            <p class="text-gray-400 mt-1">Adicione uma nova tecnologia ao seu stack</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.stack.store') }}" class="space-y-6">
        @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="level" name="level" 
                        ...
                        <option value="1" {{ old('level') == '1' ? 'selected' : '' }}>Iniciante</option>
                        <option value="2" {{ old('level') == '2' ? 'selected' : '' }}>Intermediário</option>
                        <option value="3" {{ old('level') == '3' ? 'selected' : '' }}>Avançado</option>
                        <option value="4" {{ old('level') == '4' ? 'selected' : '' }}>Expert</option>
                        <option value="5" {{ old('level') == '5' ? 'selected' : '' }}>Especialista</option>
                    </select>
                    @error('level')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
